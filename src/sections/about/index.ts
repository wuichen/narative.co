const Morph = (from, to) => {
  ...
  function applyScale(x = transform.scaleX || 1, y = transform.scaleY || 1) {
    return apply(
      () =>
        (from.style[transformString] = constructTransformString(
          'scale',
          `${x}, ${y}`
        ))
    )
  ...
  function applyTranslate(
    x = transform.transX || 0,
    y = transform.transY || 0
  ) {
    return apply(
      () =>
        (from.style[transformString] = constructTransformString(
  ...
  function applyCss(props) {
    ...
    for (const key in props) {
      if (transforms.includes(key)) {
        return apply(
          () =>
            (from.style.transform = constructTransformString(key, props[key]))
        )
      }
  ...
  function next(callbacks) {
    const complete = () => {
      ...
      if (typeof callbacks[0] === 'function') {
        ...
        console.error(
          'M.next() only accepts a list of functions or object literals.'
        )
      }
